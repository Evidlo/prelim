    ...
    where $bold(c) = {{a_(0 0), ...}, {b_(0 0), ...}, {p_(0 0), ...}, {q_(0 0), ...}, c, k}$ are model parameters.  Zoennchen et al. truncate to $l ≤ 3$ for a total of $4 × 16 + 2 = 66$ free parameters. Note that $∀l : B_(l 0)(r) := 0$ by definition.
      ...
      #math.equation(
          $
              ...
              hat(bold(rho))_"HDOF" = M(hat(bold(c)))
          $
      )
...
= Static Retrieval Validation <static_validation>
  ...
  #figure(
      table(
          ...
          table.hline(stroke: 2pt),
          [*Zoennchen* @zoennchen_new], [Static], [Data\ (TWINS)], [Quiet], [],
          [Cucho-Padin \ *TWINS* @zoennchen_new], [Static], [Data\ (TWINS)], [Quiet], [],
          [Vidal-Madjar\ & Bertaux (*VMB*)], [Dynamic], [Simulation], [Quiet], [],
          [Connor\ *MSIS*], [Dynamic], [Simulation\ (MSIS)], [Quiet], [],
          [Connor\ *TIMEGCM*], [Dynamic], [Simulation\ (TIMEGCM)], [Storm], [],
  ...
  == Retrieval Performance <retrieval_validation>
    ...
    Notably, the retrieval error is the smallest in the transverse (YZ) plane which is especially noticeable in the VMB retrieval.  This plane is oriented roughly perpendicular to measured #gls("LOS"), which intuitively should give good conditioning to voxels in this region.
  ...
  #figure(
      grid(
          ...
          subfigure(image("figures/zoennchen_coeffs.png", ..h), "quietrecon", "Zoennchen recon. coefficients"),
          subfigure(image("figures/zoennchen_direct.png", ..h), "quietrecon", "Zoennchen ground truth"),
          subfigure(image("figures/vmb_coeffs.png", ..h), "quietrecon", "VMB recon. coefficients"),
          subfigure(image("figures/vmb_direct.png", ..h), "quietrecon", "VMB ground truth"),
      ),
      caption: [#rt([FIXME: placeholder]).  Static retrieval error during geomagnetically quiet conditions shown over 3 cardinal Cartesian planes.  Monte-Carlo trials show reconstructions meet accuracy requirement with sufficiently high confidence.]
  ...
  #figure(
    ...
    === View Geometry Discretization <spb_discretization>
    ...
    === Reconstruction Model Parameters
      ...
      #figure(
          ...
          caption: "Direct Fit % Errors. Sweeping spherical harmonic order truncation and number of control points"
      ) <parameter_sweep>
...
= Dynamic Retrieval of Exosphere <dynamic_retrieval>
